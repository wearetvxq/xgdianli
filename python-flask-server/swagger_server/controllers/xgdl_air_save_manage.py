...
from swagger_server.utitl.Jwts import jwt_s
...
def ele_meter_list():
    ...
    title = '电表列表'
    resultdict['meter_list'] = meter_list
...
def get_real_time_data(meter_list=''):
    resultdict = {}

    jwt_s.getGroupMeter()
    result_list = jwt_s.getRealTime(meter_list)
    ...
    title_old = ['temperature', 'type', 'voltage', 'kWh', 'kW', 'kVar', 'timestamp', 'current', 'factor', 'devId']
    title = ['型号', '电压', '用电量(kWh)', '有功功率(kW)', '无功功率(kVar)', '时间点', '电流', '功率因素', '设备ID']
    real_time_data = [[type, voltage, kWh, kW, kVar, timestamp, current, factor, devId]]

    resultdict['real_time_data'] = real_time_data
    resultdict['title'] = title
    reps = jsonify(resultdict)
    reps.headers["Access-Control-Allow-Origin"] = "*"
    return reps
...
def cap_method(sta_name='', cap_count=3):
    ...
    cap_method.append('这是基础方案，可根据实际情况微调')  #描述
...
def capacitance_scheme(sta_name='', power_fac_old='', power_fac_new=''):
    ...
    if sta_name != '' and power_fac_old != '' and power_fac_new != '':
        ...
        no_power_list = []
        sql_1 = "select avg(3_not_active_ele) from xgdl_high_voltage_user WHERE sta_no = '{}' AND 3_not_active_ele !='' AND 3_not_active_ele !='0' GROUP BY `date` ORDER BY `date`".format(
            sta_name.split('_')[0])
        ...
        no_power_avg = sum(no_power_list) / len(no_power_list) * 3
