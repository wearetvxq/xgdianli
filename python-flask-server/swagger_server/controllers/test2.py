...
def function_1():
    return (test_page_object.hello(22,[1,32]))
...
def insert_met_reader():
    ...
    # for i in range(len(result_0)):
    #     sql = """insert into xgdl_low_valtage_user_sta_no(sta_no,city)VALUES('{0}','{1}')""".format(result_0[i][0],result_0[i][1])
    #     print('====sql:==', sql)
    #     mysql.executesql(sql)
    #     mysql.commitdata()
    ...
    # print('----result_0--', result_0)
    # for i in range(len(result_0)):
    #     sql = """insert into xgdl_high_voltage_user_sta_no(sta_no,city)VALUES('{0}','{1}')""".format(result_0[i][0],
    #                                                                                                 result_0[i][1])
    #     print('====sql:==', sql)
    #     mysql.executesql(sql)
    #     mysql.commitdata()
...
def insert_city():
    ...
    for i in range(len(city_list)):
        sql = """insert into basic_city_list(sta_no, city) VALUES('{0}', '{1}')""".format(city_list[i][0], city_list[i][1])
        print('====sql:==', sql)
        mysql.executesql(sql)
        mysql.commitdata()
...
def city_sta():
    ...
    sql = 'SELECT sta_num FROM Predict_sta_load GROUP BY sta_num'
    result = mysql.query(sql)

    for i in range(len(result)):
        sql_1 = "SELECT city FROM basic_city_list WHERE sta_no ='{}'".format(result[i][0])
        result_1 = mysql.query(sql_1)
        # print('----',len(result_1))
        if len(result_1) < 1:
            print('0')
