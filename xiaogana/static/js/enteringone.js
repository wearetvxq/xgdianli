app.controller('enteringoneCtrl', function($scope,$http,$filter,$state,$rootScope) {
    ...
    function maxPag(){
        ...
        function loadList(PageIndex){
            $.ajax({
                url: zsurl+'/xiaogan/table_device_management_chm',
                type : "POST",
                dataType: "json",
                async: false, 
                data : {
                    choose_city:$scope.sAbnOne,
                    sta_name:$scope.sAbnTwo,
                    PageIndex:PageIndex
                },
                success : function(response){
        ...
        $scope.look=function(item){
            ...
            $state.go('entering.enteringdetail');//显示详情
        };
    }
});
